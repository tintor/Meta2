...
public final class Plane3 {
	...
	public static Plane3 create(final float a, final float b, final float c, final float d) {
		return new Plane3(Vector3.unit(a, b, c), d);
	}

	/** @return plane from normal and point on plane */
	public static Plane3 create(final Vector3 normal, final Vector3 point) {
		final Vector3 unitNormal = normal.unit();
		return new Plane3(unitNormal, -unitNormal.dot(point));
	}

	/** @return plane from normal and offset */
	public static Plane3 create(final Vector3 normal, final float offset) {
		return new Plane3(normal.unit(), offset);
	}
	...
	public static Vector3 closest(final Vector3 p, final Vector3 a, final Vector3 b, final Vector3 c) {
		...
		return Vector3.create(p.x - nx * d, p.y - ny * d, p.z - nz * d);
	}

	// Fields
	/** unit normal of the plane */
	public final Vector3 unitNormal;
	/** offset = plane_function(Vector3.Zero) */
	public final float offset;

	// Constructors
	private Plane3(final Vector3 unitNormal, final float offset) {
	...
	public Vector3 closest(final Vector3 a) {
	...
	public Plane3 move(final float a) {
		return new Plane3(unitNormal, offset - a);
	}

	/** @return plane with reversed sides */
	public Plane3 flip() {
		return new Plane3(unitNormal.neg(), -offset);
	}
