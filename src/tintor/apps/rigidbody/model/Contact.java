...
import tintor.apps.rigidbody.model.solid.Material;
...
public class Contact implements Constraint {
	...
	public final Material materialA, materialB;
	...
	public Contact(final Body bodyA, final Body bodyB, final Material materialA, final Material materialB,
			final Vector3 normal, final Vector3 point, final float depth, final Arbiter arbiter) {
		this.bodyA = bodyA;
		this.bodyB = bodyB;
	...
	public void prepare(final float dt) {
		ra = point.sub(bodyA.transform().offset);
		rb = point.sub(bodyB.transform().offset);

		K = Body.imassAt(bodyA, bodyB, ra, rb);
		nK = normal.mul(K);
		invK = K.inv();

		elasticity = elasticity(materialA.elasticity, materialB.elasticity);
		sfriction = friction(materialA.sfriction, materialB.sfriction);
		dfriction = friction(materialA.dfriction, materialB.dfriction);
	...
	public void processContact(final float e) {
		...
		if (nVel < 0) {
			...
			if (nJ < 0 || J.sub(nJ, normal).square() > sfriction * nJ * sfriction * nJ) {
